...
const hmb=l=>`<h1>${l==="es"?"Calculadoras online gratis":"Free online calculators"}</h1><p class="desc">${U.tag[l]}</p><input id="q" class="q" placeholder="${l==="es"?"Buscar…":"Search…"}">`+C.map(c=>`<h2>${(H1[c.id]||{})[l]||c[l]}</h2><div class="grid">${T.filter(t=>t.cat===c.id).map(t=>tile(l,t)).join("")}</div>`).join("")+`<h2>${U.pop[l]}</h2><div class="grid">${POP.map(id=>tile(l,gid(id))).join("")}</div>`;
const paths=[];
L.forEach(l=>{
