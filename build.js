...
const hmb=l=>`<h1>${l==="es"?"Calculadoras online gratis":"Free online calculators"}</h1><p class="desc">${U.tag[l]}</p><input id="q" class="q" placeholder="${l==="es"?"Buscar…":"Search…"}"><div class="grid">${C.map(c=>`<a class="tile" href="/${l}/${c.id}/"><b>${c[l]}</b><small style="display:block;color:var(--mut);font-size:12px;margin-top:4px">${T.filter(t=>t.cat===c.id).length} ${U.all[l]}</small></a>`).join("")}</div><h2>${U.pop[l]}</h2><div class="grid">${POP.map(id=>tile(l,gid(id))).join("")}</div>`;
const paths=[];
L.forEach(l=>{
